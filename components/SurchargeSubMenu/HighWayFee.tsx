...
import DetailedManualFareResult from './DetailedManualFareResult';
...
export default function DetailedManualFareResult({
  ...
  // 合計料金計算（高速道路利用料金を追加）
  const totalCharges = (charges.highWayFee || 0) + // ← 高速道路利用料金を最初に追加
                      (charges.departureWaitingFee || 0) + 
    ...
    <div style={{
      ...
      {totalCharges > 0 && (
        <div style={{ margin: "16px 0", lineHeight: 1.5, overflow: "hidden" }}>
          ...
          <dl style={{ margin: "12px 0", lineHeight: 1.5, overflow: "hidden" }}>
            {/* 高速道路利用料金を最上段に追加 */}
            {charges.highWayFee !== undefined && charges.highWayFee > 0 && (
              <>
                <dt style={{ float: "left", clear: "left", width: 150 }}>高速道路利用料金</dt>
                <dd style={{ marginLeft: 180 }}>
                  ¥{charges.highWayFee.toLocaleString()}
